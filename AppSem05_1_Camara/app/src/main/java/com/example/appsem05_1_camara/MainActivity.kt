...
class MainActivity : AppCompatActivity() {
    ...
    private fun requestCameraPermission() {
        if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.CAMERA)){
            ...
            //Si el usuario nunca ha aceptado ni rechazado, le pedimos que acpete el permiso
            Toast.makeText(this, "Debe aceptar permiso", Toast.LENGTH_SHORT).show()
        ...
        when (requestCode){
            CAMERA_REQUEST_CODE -> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    //permiso acceptado por usuario
                    //Llamaria a cualquier funcionalidad que quiera hacer
                    Toast.makeText(this, "Se autorizo acceso a camara", Toast.LENGTH_SHORT).show()
                }
                else {
